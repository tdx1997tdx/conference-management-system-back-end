...
public class MeetingControler {
    ...
    public String meetingSearch(@RequestBody JSONObject jsonParam){
        Meeting meeting = JSON.parseObject(jsonParam.toString(), Meeting.class);
        Filter.attributeFilter(meeting);
        List<Meeting> result = meetingService.meetingSearchService(meeting);
    ...
    public String meetingGetAll(){
        List<Meeting> result = meetingService.meetingGetService();
        return JSON.toJSONString(result);
    }
    ...
    public String meetingOrderSearch (@RequestBody JSONObject jsonParam){
        Meeting meeting = JSON.parseObject(jsonParam.toString(), Meeting.class);
        List<Meeting> result = meetingService.meetingOrderService(meeting);
        return JSON.toJSONString(result);
    }
